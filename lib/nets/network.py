...
class Network(nn.Module):
    ...
    def _init_modules(self):
        ...
        self.cls_score_net = nn.Linear(self._fc7_channels, 61)

        # self.bbox_pred_net = nn.Linear(self._fc7_channels,
        #                                self._num_classes * 4)
        self.bbox_pred_net = nn.Linear(self._fc7_channels, 4)
    ...
    def forward(self, image, im_info, gt_boxes=None, mode='TRAIN'):
        ...
        if mode == 'TEST':
            stds = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_STDS).unsqueeze(0).expand_as(bbox_pred)
            means = bbox_pred.data.new(cfg.TRAIN.BBOX_NORMALIZE_MEANS).unsqueeze(0).expand_as(bbox_pred)
