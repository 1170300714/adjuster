...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        #self._COCO_ani = COCO(self._get_ann_ani_file())
      
        cats = self._COCO.loadCats(self._COCO.getCatIds())
        self._classes = tuple(['__background__'] + [c['name'] for c in cats])
        self._class_to_ind = dict(list(zip(self.classes, list(range(self.num_classes)))))
        self._ind_to_class = dict(list(zip(list(range(self.num_classes)),self.classes)))
       
        print(self._class_to_ind.keys())
        exit()
    ...
    def _get_ann_file(self):
        prefix = 'instances60' if self._image_set.find('test') == -1 else 'image_info'
        #prefix = 'instances' if self._image_set.find('test') == -1 else 'image_info'
        return osp.join(self._data_path, 'annotations',
                        prefix + '_' + self._image_set + self._year + '.json')

    def _get_ann_ani_file(self):
        prefix = 'instances60_animals' if self._image_set.find('test') == -1 else 'image_info'
        #prefix = 'instances' if self._image_set.find('test') == -1 else 'image_info'
        return osp.join(self._data_path, 'annotations',
                        prefix + '_' + self._image_set + self._year + '.json')
    ...
    def _load_image_ani_set_index(self):
        """
    Load image ids.
    """
        image_ids = self._COCO_ani.getImgIds()
...
class coco_60(imdb):
  def __init__(self, image_set, year):
    imdb.__init__(self, 'coco60_' + year + '_' + image_set)
    # COCO specific config options
    self.config = {'use_salt': True,
                   'cleanup': True}
    # name, paths
    ...
    self._split_ids = list(set(self._valid_ids) - set(self._voc_ids))
    print(self._split_ids)
    # cats = self._COCO.loadCats(self._COCO.getCatIds())
    cats = self._COCO.loadCats(self._split_ids)
    self._classes = tuple(['__background__'] + [c['name'] for c in cats])
    self._class_to_ind = dict(list(zip(self.classes, list(range(self.num_classes)))))
  ...
  def _get_ann_file(self):
    prefix = 'instances60' if self._image_set.find('test') == -1 \
      else 'image_info'
    return osp.join(self._data_path, 'annotations',
                    prefix + '_' + self._image_set + self._year + '.json')
