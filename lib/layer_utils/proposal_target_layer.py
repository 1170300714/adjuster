...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
  ...
  """
    ...
    # print("clss size: ", clss.size())
    # print(clss)
    bbox_targets = clss.new_zeros(clss.numel(), 4)
    # print("bbox targets size: ", bbox_targets.size())
    bbox_inside_weights = clss.new_zeros(bbox_targets.shape)
    # print("bbox inside weights size: ", bbox_inside_weights.size())
    ...
    if inds.numel() > 0:
        ...
        # bbox_targets[dim1_inds, :] = bbox_target_data[inds][:, 1:]
        # bbox_inside_weights[dim1_inds, :] = bbox_targets.new(
        #     cfg.TRAIN.BBOX_INSIDE_WEIGHTS).view(-1, 4).expand_as(dim1_inds)
        bbox_targets[inds, :] = bbox_target_data[inds][:, 1:]
        bbox_inside_weights[inds, :] = bbox_targets.new(
            cfg.TRAIN.BBOX_INSIDE_WEIGHTS).view(-1, 4).expand_as(dim1_inds)
